...
interface FixedBillsTabContentProps {
  potentialFixedBills: PotentialFixedBill[];
  onToggleFixedBill: (bill: PotentialFixedBill, isChecked: boolean) => void;
}

export function FixedBillsTabContent({ potentialFixedBills, onToggleFixedBill }: FixedBillsTabContentProps) {
  if (potentialFixedBills.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 opacity-30">
        <Calendar className="w-12 h-12 mb-3" />
        <p className="text-[10px] font-black uppercase tracking-widest">Nenhuma parcela fixa encontrada</p>
      </div>
    );
  }
      ...
      {potentialFixedBills.map((bill) => (
        ...
        >
          <div className="flex items-start justify-between mb-3">
            ...
            {bill.isIncluded && (
              <div className="w-5 h-5 rounded-full bg-primary flex items-center justify-center text-primary-foreground animate-in zoom-in duration-300">
          ...
          <div className="mt-3 pt-3 border-t border-border/20 flex items-end justify-between">
            <div>
              ...
              <p className={cn("text-base font-black", bill.isIncluded ? "text-primary" : "text-foreground")}>
