...
export function LoanDetailDialog({ emprestimo, open, onOpenChange }: LoanDetailDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {isMobile ? (
        <DialogContent className="fixed inset-0 max-w-full h-full rounded-none p-0 overflow-hidden flex flex-col bg-card">
          <DialogHeader className="p-6 shrink-0 bg-muted/20 border-b relative">
            ...
            <div className="flex items-center justify-between pl-12">
              <div className="flex items-center gap-4">
                <div className="p-3 rounded-[1.25rem] bg-primary/10 text-primary shadow-inner"><Building2 className="w-6 h-6" /></div>
                <div className="space-y-1">
                  <DialogTitle className="text-xl font-black tracking-tight">{emprestimo.contrato}</DialogTitle>
              ...
              {!isPending && !isEditing && (
                <Button variant="ghost" size="icon" onClick={() => setIsEditing(true)} className="rounded-full bg-primary/10 text-primary"><Edit className="w-5 h-5" /></Button>
              )}
            </div>
          </DialogHeader>
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            {showConfigForm ? (
              ...
              <Tabs defaultValue="geral" className="flex-1 flex flex-col min-h-0">
                <TabsList className="bg-muted/30 h-14 border-b rounded-none px-4 gap-4 overflow-x-auto no-scrollbar justify-start">
                  ...
                  <div className="p-6 space-y-8 pb-12">
                    <TabsContent value="geral" className="mt-0 space-y-8 focus-visible:outline-none">
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          { l: 'Saldo', v: formatCurrency(calculos.saldoDevedor), c: isQuitado ? 'text-success' : 'text-destructive', i: TrendingDown },
                          { l: 'Parcela', v: formatCurrency(emprestimo.parcela), c: 'text-warning', i: Calendar },
                          { l: 'Progresso', v: `${calculos.percentualQuitado.toFixed(0)}%`, c: 'text-primary', i: Award },
                          { l: 'Vencimento', v: calculos.dataFinal.toLocaleDateString('pt-BR'), c: 'text-foreground', i: Target }
                        ].map((item, idx) => (
                          <div key={idx} className="p-4 rounded-[1.5rem] bg-muted/20 border border-border/40">
                            <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest mb-1">{item.l}</p>
                            <p className={cn("text-sm font-black tabular-nums", item.c)}>{item.v}</p>
                          </div>
                        ))}
                      </div>
                      <div className="p-6 rounded-[2rem] bg-success/5 border border-success/20 space-y-4 text-center">
                        <Zap className="w-6 h-6 text-success mx-auto" />
                        <div>
                          <p className="text-[10px] font-black text-success uppercase tracking-widest">Economia com Quitação</p>
                          <p className="text-3xl font-black text-success tabular-nums">{formatCurrency(calculos.economiaQuitacao)}</p>
                        </div>
                        <Button className="w-full rounded-full h-12 font-black text-xs">SIMULAR QUITAÇÃO</Button>
                      </div>
                    </TabsContent>
                    ...
                    <TabsContent value="analise" className="mt-0 space-y-10">
        ...
        <ResizableDialogContent
          ...
          className="bg-card border-none shadow-2xl p-0 overflow-hidden flex flex-col"
        >
